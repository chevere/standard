...
final class ArrayFilterFunctionsTest extends TestCase
{
    ...
    public function testArrayFilterKey(): void
    {
        ...
        $arrayFilterKey = array_filter(array: $array, mode: ARRAY_FILTER_USE_KEY);
        $expected = $array;
        $this->assertSame($expected, $arrayFilterKey);
        $arrayFilterKey = arrayFilterKey($array);
        $expected = [
            1 => 'b',
        ];
        $this->assertSame($expected, $arrayFilterKey);
    ...
    public function testArrayFilterBoth(): void
    {
        ...
        $arrayFilterBoth = array_filter(array: $array, mode: ARRAY_FILTER_USE_BOTH);
        $expected = $array;
        $this->assertSame($expected, $arrayFilterBoth);
        $arrayFilterBoth = arrayFilterBoth($array);
        $this->assertSame($expected, $arrayFilterBoth);
        $array = [
            'a' => 1,
            'b' => [
        ...
        $arrayFilterBoth = arrayFilterBoth($array, function ($v, $k) {
            return $k !== 'c' && $v !== 1;
        });
        $this->assertSame($expected, $arrayFilterBoth);
    }
